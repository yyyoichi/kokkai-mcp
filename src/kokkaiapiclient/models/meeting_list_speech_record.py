from __future__ import annotations
from collections.abc import Callable
from dataclasses import dataclass, field
from kiota_abstractions.serialization import AdditionalDataHolder, Parsable, ParseNode, SerializationWriter
from typing import Any, Optional, TYPE_CHECKING, Union

if TYPE_CHECKING:
    from .meeting_list_speech_record_speaker import MeetingListSpeechRecord_speaker

@dataclass
class MeetingListSpeechRecord(AdditionalDataHolder, Parsable):
    # Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.
    additional_data: dict[str, Any] = field(default_factory=dict)

    # 発言者名。
    speaker: Optional[MeetingListSpeechRecord_speaker] = None
    # 発言ID。"issueID_発言番号"形式。
    speech_i_d: Optional[str] = None
    # 発言順序。
    speech_order: Optional[int] = None
    # 発言テキストへのURL。
    speech_u_r_l: Optional[str] = None
    
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> MeetingListSpeechRecord:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: MeetingListSpeechRecord
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return MeetingListSpeechRecord()
    
    def get_field_deserializers(self,) -> dict[str, Callable[[ParseNode], None]]:
        """
        The deserialization information for the current model
        Returns: dict[str, Callable[[ParseNode], None]]
        """
        from .meeting_list_speech_record_speaker import MeetingListSpeechRecord_speaker

        from .meeting_list_speech_record_speaker import MeetingListSpeechRecord_speaker

        fields: dict[str, Callable[[Any], None]] = {
            "speaker": lambda n : setattr(self, 'speaker', n.get_object_value(MeetingListSpeechRecord_speaker)),
            "speechID": lambda n : setattr(self, 'speech_i_d', n.get_str_value()),
            "speechOrder": lambda n : setattr(self, 'speech_order', n.get_int_value()),
            "speechURL": lambda n : setattr(self, 'speech_u_r_l', n.get_str_value()),
        }
        return fields
    
    def serialize(self,writer: SerializationWriter) -> None:
        """
        Serializes information the current object
        param writer: Serialization writer to use to serialize this model
        Returns: None
        """
        if writer is None:
            raise TypeError("writer cannot be null.")
        writer.write_object_value("speaker", self.speaker)
        writer.write_str_value("speechID", self.speech_i_d)
        writer.write_int_value("speechOrder", self.speech_order)
        writer.write_str_value("speechURL", self.speech_u_r_l)
        writer.write_additional_data_value(self.additional_data)
    

